 ...
 *   - "off":     skip the LLM entirely (orchestrator uses its deterministic composer).
 */
...
export async function complete(
  ...
  for (const p of providers) {
    try {
      ...
      console.warn(`[llm] provider "${p.name}" failed:`, (err as Error).message);
    }
  }
  return null;
}
...
export async function* completeStream(
  ...
  for (const p of providers) {
    try {
      ...
      console.warn(
        `[llm] stream provider "${p.name}" failed:`,
