...
function toProperty(p, currency) {
...
export async function searchGoogleHotels(opts) {
  ...
  try {
    ...
    if (!res.ok) {
      appLogger.warn(`[serp/hotels] HTTP ${res.status}`);
      return [];
    }
    const json = await res.json();
    if (json.error) {
      appLogger.warn(`[serp/hotels] ${json.error}`);
      return [];
    }
    ...
    appLogger.warn(`[serp/hotels] failed: ${e instanceof Error ? e.message : e}`);
    return [];
  } finally {
    clearTimeout(timer);
  }
